...
class FTPUploader():
	...
	def showSettingsUI(self, parentWidget):
		...
		if self.includeFileExtension:
			self.settingsDialog.group_clipboard.radio_includefileextension.checked = True
		else:
			self.settingsDialog.group_clipboard.radio_excludefileextension.checked = "false"
	...
	def loadSettings(self):
		...
		self.includeFileExtension = settings.value("include-file-extension", "true") in ['true', True]
		settings.endGroup()
		settings.endGroup()
	...
	def saveSettings(self):
		...
		settings.setValue("include-file-extension", self.settingsDialog.group_clipboard.radio_includefileextension.checked)
		settings.endGroup()
		settings.endGroup()
	...
	def upload(self, screenshot, name):
		...
		f.close()
		if self.url:
			if not self.includeFileExtension and ".{}".format(ScreenCloud.getScreenshotFormat()) in name:
				name = self.url + ScreenCloud.formatFilename(name[:name.index('.')], False)
			else:
				name = self.url + ScreenCloud.formatFilename(name)
			ScreenCloud.setUrl(name)
		return True
